var sender_id = Math.random().toString(36).substring(+2, 6) + Math.random().toString(36).substring(2, 6);

var alreadyClicked = false;

var error_text = ["Please enter a valid email address."];
// console.log(text);
//Bot pop-up intro
...
function restartConversation() {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            hideBotTyping();
            fallbackMsg = "The European Chatbot & Conversational AI Summit- 100% virtual/Online";
            BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
            $(BotResponse).appendTo(".chats").show().fadeIn(1000);
            showBotTyping();

            setTimeout(function() {
                hideBotTyping();
                fallbackMsg = "<img src='https://conversio-bot-media.s3.amazonaws.com/UserId2331/1600087111-European%20Chatbot%20Banner%20Changed%20current.png.pub.png' width=190 >";
                BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
                $(BotResponse).appendTo(".chats").show().fadeIn(1000);
                showBotTyping();
                ...
                setTimeout(function() {
                    hideBotTyping();
                    fallbackMsg = "We are meeting virtually online on March 4 & 5, 2021";
                    BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
                    $(BotResponse).appendTo(".chats").show().fadeIn(1000);
                    showBotTyping();
                    scrollToBottomOfResults();
                    ...
                    setTimeout(function() {
                        hideBotTyping();
                        fallbackMsg = "The European Chatbot & Conversational AI Summit is a two days virtual conference and exhibition designed to host industry executives, and adopters of Conversational AI, Chatbots, speech or voice technology, UX design , NLP & Virtual Assistant . The summit will reflect the latest tendencies and recent application changes in the Conversational AI space in the European market <br/> <br/>.We believe conversational AI will transform millions of lives in Europe and our goal is to help form a better ecosystem. We do so by organizing series of international conferences in Europe by bringing the leading professionals and organisations who design, build, and market conversational AI based technologies.<br/> <br/>The European Chatbot & Conversational AI Summit will feature insightful talks and solution based discussions that will help to understand the concept of Bots,Conversational A.I,virtual assistant,NLC, NLP, UX Design and Voice associated benefits and scrutinize implementation challenges and their solutions.";
                        BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
                        $(BotResponse).appendTo(".chats").show().fadeIn(1000);
                        showCustomBotTyping();
                        // scrollToBottomOfResults();
                        scrollToCustomResults();

                        setTimeout(function() {
                            hideBotTyping();
                            fallbackMsg = "May I know your name please?";
                            BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
                            $(BotResponse).appendTo(".chats").show().fadeIn(1000);
                            // scrollToBottomOfResults();
...
function scrollToCustomResults() {

    var terminalResultsDiv = document.getElementById("chats");
    terminalResultsDiv.scrollTop = 150;
}

var doc = document.getElementById('userInput');
var last_bot_message = ""

// Changing the Input Type
function change_input_dom_type(message){
    if (message.includes('email')){
        doc.type = "email"
        doc.placeholder = "Enter your email address"
    }else{
        doc.type = "text"
        doc.placeholder = "Type a message..."
    }
}
...
function send(message) {
    console.log(JSON.stringify({ message: message, sender: sender_id }));
    $.ajax({
        url: "http://d5bbf2e45604.ngrok.io/fundi_webhook",
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({ text: message, recipient_id: sender_id }),
        success: function(botResponse, status) {
            // console.log("Response from Rasa: ", botResponse, "\nStatus: ", status);
            ...
            last_bot_message = botResponse[botResponse.length-1].text;
            console.log(botResponse);
            change_input_dom_type(last_bot_message)
...
$(".usrInput").on(" keyup keypress", function(e) {
    ...
    if (keyCode === 13) {
        ...
        if (text == "" || $.trim(text) == "") {
            ...
            if (last_bot_message.includes("email")){
                if(validateEmail(text)){
                    send(text);
                    return;
                }else{
                    setBotResponse(error_text);
                    return;
                }
            }
            send(text);
...
function setBotResponse(response) {
    ...
    setTimeout(function() {
        ...
        if (response.length == 1){
            var BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + response.toString() + '</p><div class="clearfix"></div>';
...
$("#profile_div").click(function() {
    ...
    if(!alreadyClicked){
        ...
        // var introTexts = ['Welcome', 'My name is Siri', "Hureeeeh"];
    
        // for (var i = 0; i < introTexts.length; i++) {
        //     setTimeout(function() {
        //         hideBotTyping();
        //         var fallbackMsg = introTexts[i];
        //         var BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
        //         $(BotResponse).appendTo(".chats").show().fadeIn(1000);
        //         showBotTyping();
        //     }, i*1000);
...
function addSuggestion(textToAdd) {
    setTimeout(function() {
        ...
        var book_ticket = 'https://www.quicket.co.za/events/106575-chatbot-africa-summit-2021#/'
        var book_speaker = 'http://chatbotafrica.com/form'

        var btn_array = []
        var split_array = suggestions[0].payload.split("\"")
        btn_array[0] = split_array[split_array.length-2]
        var split_array = suggestions[1].payload.split("\"")
        btn_array[1] = split_array[split_array.length-2]

        suggestions[0].payload = book_ticket;
        suggestions[1].payload = book_speaker;
        for (i = 0; i < suggLength; i++) {
			$('<div class="menuChips" id=\''+(btn_array[i])+'\' data-payload=\''+(suggestions[i].payload)+'\'>' + suggestions[i].title + "</div>").appendTo(".menu");
		}
        scrollToBottomOfResults();
    }, 1000);
}
...
$(document).on("click", ".menu .menuChips", function() {
    ...
    window.open(payload);
    // setUserResponse(text);
    console.log("payload value: ", this.id)
    send(this.id);
